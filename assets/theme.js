...
theme.config = {
  cssLoaded: false,
  // bpSmall: false, /* 25.08.22 ID - Mobile nav */
  bpSmall: true,
  hasSessionStorage: true,
  // mediaQuerySmall: 'screen and (max-width: 768px)', /* 25.08.22 ID - Mobile nav */
  mediaQuerySmall: 'screen and (max-width: 300px)',
  youTubeReady: false,
  vimeoReady: false,
  vimeoLoading: false,
...
(function(){
  ...
  theme.Drawers = (function() {
    ...
    Drawers.prototype = Object.assign({}, Drawers.prototype, {
      ...
      close: function(evt) {
        let cartDrawerSuccessMessage = document.querySelectorAll('.cartDrawerSuccessMessage');        
        if (!this.isOpen) {
          return;
        }
        ...
        if (cartDrawerSuccessMessage) {
          cartDrawerSuccessMessage.forEach(function(el) {
  ...
  theme.AjaxProduct = (function() {
    ...
    ProductForm.prototype = Object.assign({}, ProductForm.prototype, {
      addItemFromForm: function(evt, callback){
        ...
        var crosssells = this.form.closest('.pdpMeta').querySelectorAll('.crosssellCheckbox');
        var crosssellItemsArray = new Array;
        crosssells && crosssells.forEach((el, i) => {
          let variantId = Number(el.dataset.variantId);
          if (el.checked) {
            crosssellItemsArray.push({'id':variantId,'quantity':1,'properties':{'_added_as_crosssell':'yes'}});
          }
        });

        if (crosssellItemsArray.length > 0) {
          var dataValues = new URLSearchParams(data);
          crosssellItemsArray.push({'id':dataValues.get('id'),'quantity':1,'properties':{'_added_as_crosssell':'yes'}});
          data = JSON.stringify({'items':crosssellItemsArray});
          requestHeaders = {
            'Content-Type': 'application/json',
            'X-Requested-With': 'XMLHttpRequest'
          };
  ...
  theme.slideNav = (function() {
    ...
    function init() {
      ...
      closeBtns.forEach(btn => {
        btn.addEventListener('click', function() {
          ...
          navWrapper.classList.remove('is-transitioning');
    ...
    function closeNav() {
      console.log(activeMenuToggle);
      toggleBtns.forEach(btn => {
        btn.classList.remove(classes.isActive);
      });
  
      theme.utils.prepareTransition(navWrapper, function() {
        navWrapper.classList.remove('nav-is-open');
      });
      ...
      window.off('keyup' + namespace);
      document.documentElement.classList.remove(classes.isOpen);
    }
  ...
  theme.headerNav = (function() {
    ...
    function init() {
      ...
      // A/B Test only
      if (document.querySelector('#shopify-section-header-new').offsetParent !== null) {
        wrapper = document.querySelector('#shopify-section-header-new .header-wrapper');
        siteHeader = document.querySelector('#shopify-section-header-new .site-header');
      }
      ...
      window.on('scroll' + config.namespace, theme.utils.throttle(10, stickyMenuOnScroll));
      // Make sure it fires after scrolling, to be safe and sure
      // the height is properly set
      window.on('scroll' + config.namespace, theme.utils.debounce(10, stickyMenuOnScroll));
      ...
      init: function() {
        ...
        if (slides.length > 1) {
          var sliderArgs = {
            ...
            fade: this.slideshow.dataset.sliding === 'true' ? false : true,
  ...
  DOMready(function(){
    ...
    const cartDrawerUpsellsGlob = document.querySelector('.cartDrawerUpsells');
    const customATCForm = document.querySelectorAll('.customATCForm');
    const customATCFormNumber = customATCForm ? customATCForm.length : 0;
    var customATCFormActiveCount = 0;
    customATCForm && customATCForm.forEach(function(form) {
      if (form.classList.contains('d-none')) {
        customATCFormActiveCount += 1;
      }
      let atcBtn = form.querySelector('.customATCBtn');
      let prodId = form.dataset.productId ?? null;
      let varId = form.dataset.variantId ?? null;
      if (atcBtn && prodId !== null && varId !== null) {
        atcBtn.addEventListener('click', function (e) {
          e.preventDefault();
          let serializedData = 'form_type=product&quantity=1&utf8=%E2%9C%93&data-product-id=' + prodId + '&id=' + varId;
          fetch(theme.routes.cartAdd, {
            method: 'POST',
            body: serializedData,
            credentials: 'same-origin',
          ...
          .then(function(data) {
            if (data.status === 422) {
              ...
              let customATCFormCount = customATCForm ? customATCForm.length : 0;
              var customATCFormPassed = 0;
              customATCForm && customATCForm.forEach(function(form) {
                form.classList.contains('d-none') && customATCFormPassed++;
              });
    ...
    if (cartDrawerUpsellsGlob) {
      if (customATCFormNumber <= customATCFormActiveCount) {
        cartDrawerUpsellsGlob.classList.add('d-none');
      } else {
...
const loadScriptsInterval = setInterval(function() {
  if (window._swat) {
    ...
    window._swat.evtLayer.addEventListener('sw:addedtocart', () => {
      // Completely reinit and refresh the cart when added from wishlist
      let CartDrawerForm = document.querySelector('#CartDrawerForm');
      let newCartDrawerForm = new theme.CartForm(CartDrawerForm);
      newCartDrawerForm.buildCart();
      document.body.classList.add('cart-has-items');
    });
  }
}, 500);

var patternsPopup = new theme.Modals('PatternsPopup', 'patterns-popup', {
  closeOffContentClick: true,
  open: '.patternsPopupOpen',
  solid: true
});